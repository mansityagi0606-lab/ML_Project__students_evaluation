...
class TestOptions:
    ...
    def test_raises_accidental_env_marker_misconfig(self, config, tmpdir):
        ...
        match = (
            r"One of the parsed requirements.txt in `(install_requires|extras_require.+)` "
    ...
    def test_requirements_file(self, tmpdir):
        fake_env(
            ...
            DALS(
                """
            [options]
            install_requires = file:requirements.txt.txt
            [options.extras_require]
            colors = file:requirements.txt-extra.txt
            """
            ),
        )

        tmpdir.join('requirements.txt.txt').write('\ndocutils>=0.3\n\n')
        tmpdir.join('requirements.txt-extra.txt').write('colorama')
